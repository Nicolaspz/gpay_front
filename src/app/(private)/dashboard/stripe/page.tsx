...
export default function AdminStripeDashboard() {
  ...
  const { data: summaries = [], isLoading: loading } = useQuery({
    queryKey: ['stripe-summaries'],
    queryFn: async () => {
      const res = await axios.get(`${stripeBaseUrl}/transactions/admin/clients`, {
  ...
  const { data: transactions = [], isLoading: loadingTx } = useQuery({
    queryKey: ['stripe-transactions', selectedUser],
    queryFn: async () => {
      if (!selectedUser) return [];
      const res = await axios.get(`${stripeBaseUrl}/transactions/user/${selectedUser}`, {
        headers: { Authorization: stripeToken }
      });
      return res.data;
    },
  ...
  const liquidateMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (selectedUser === variables.userId) {
        queryClient.invalidateQueries({ queryKey: ['stripe-transactions', selectedUser] });
      }
    },
    onError: () => {
  ...
  function viewClientTransactions(userId: string) {
